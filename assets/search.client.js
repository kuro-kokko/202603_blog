...
function renderResults(results) {
  ...
  return `<ul class="search-results__list">${results.map(
    (r) => `
    <li class="search-result-card">
      ...
      ${r.tags.length > 0 ? `<ul class="search-result-card__tags">${r.tags.map(
      (t) => `<li><a href="/tags/${encodeURIComponent(t)}/" class="tag-badge tag-badge--sm">${escHtml(t)}</a></li>`
    ).join("")}</ul>` : ""}
    </li>`
  ).join("")}</ul>`;
}
...
async function onInput(query, resultsEl) {
  if (!query.trim()) {
    resultsEl.innerHTML = "";
    return;
  }
